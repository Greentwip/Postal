...
#include "common/RSPiX_dll_common.hpp"

////////////////////////////////////////////////////////////////////////////////
// Blue API
////////////////////////////////////////////////////////////////////////////////

RSPIX_DECLSPEC extern int16_t rspInitBlue(void);

RSPIX_DECLSPEC extern void rspKillBlue(void);

#define RSP_DOSYSTEM_HOGCPU		0
#define RSP_DOSYSTEM_TOLERATEOS	1
#define RSP_DOSYSTEM_SLEEP			2

RSPIX_DECLSPEC extern void rspSetDoSystemMode(
	int16_t sMode);									// In:  Mode (use RSP_DOSYSTEM_* macros)

RSPIX_DECLSPEC extern void rspDoSystem(void);
...
RSPIX_DECLSPEC extern void rspTrace(char* szFrmt, ...);
...
// automatically removed in "release" versions of the program.
RSPIX_DECLSPEC extern int16_t rspAssert(	// Returns result.
...
RSPIX_DECLSPEC extern void rspGetMouse(
...
RSPIX_DECLSPEC extern void rspSetMouse(
	int16_t sX,				// New x position
	int16_t sY);				// New y position

RSPIX_DECLSPEC extern int16_t rspGetMouseEvent(	// Returns 0 if no event was available, non-zero otherwise
...
RSPIX_DECLSPEC extern int16_t rspGetLastMouseEvent(	// Returns 0 if no event was available, non-zero otherwise
	int16_t* psX,								// Event's X position is returned here (unless NULL)
	int16_t* psY,								// Event's Y position is returned here (unless NULL)
	int16_t* psButton,						// Event's button status is returned here (unless NULL)
	int32_t* plTime = NULL,					// Event's time stamp returned here (unless NULL)
...
RSPIX_DECLSPEC extern void rspClearMouseEvents(void);

RSPIX_DECLSPEC extern void rspHideMouseCursor(void);

RSPIX_DECLSPEC extern void rspShowMouseCursor(void);
...
RSPIX_DECLSPEC extern int16_t rspGetMouseCursorShowLevel(void);

// Set the current cursor show level.  A level of 1 or greater means the cursor
// is currently showing, while a level of 0 or less means the cursor is hidden.
RSPIX_DECLSPEC extern void rspSetMouseCursorShowLevel(
...
RSPIX_DECLSPEC extern void rspScanKeys(
	uint8_t* pucKeys);				// Out: Array of 128 unsigned chars (one per SK code)

RSPIX_DECLSPEC extern void rspClearKeyEvents(void);

RSPIX_DECLSPEC extern int16_t rspGetKey(						// Returns 1 if key was available, 0 if not
	int32_t* plKey,								// Out: Key info (0 if no key was available)
	int32_t* plTime = NULL);					// Out: Key's time stamp (unless NULL)

RSPIX_DECLSPEC extern int16_t rspIsKey(void);				// Returns 1 if key is available, 0 if not
...
// clears of the array.
RSPIX_DECLSPEC extern U8* rspGetKeyStatusArray(void);	// Returns pointer to 128-byte key status array
...
RSPIX_DECLSPEC extern int32_t rspGetToggleKeyStates(void);	// Returns toggle key state flags
...
// previous.
RSPIX_DECLSPEC extern void rspUpdateJoy(
...
RSPIX_DECLSPEC extern void rspGetJoyPos(
	int16_t sJoy,					// In:  Joystick to query.
...
// This function returns directions in an analog format (0..0xFFFF).
RSPIX_DECLSPEC extern void rspGetJoyPrevPos(
	int16_t sJoy,					// In:  Joystick to query.
	int32_t *plX,					// Out: X axis position of joystick, if not NULL.
	int32_t *plY = NULL,			// Out: Y axis position of joystick, if not NULL.
	int32_t *plZ = NULL,			// Out: Z axis position of joystick, if not NULL.
...
RSPIX_DECLSPEC extern void rspGetJoyState(
	int16_t sJoy,					// In:  Joystick to query.
...
// This function returns directions in a digital format (up, down, centered).
RSPIX_DECLSPEC extern void rspGetJoyPrevState(
	int16_t sJoy,					// In:  Joystick to query.
	U32*	pu32Buttons,		// Out: Buttons that are down, if not NULL.
									// An RSP_JOY_BUT_## bit field that is set indicates
									// that button is down.
...
RSPIX_DECLSPEC extern bool GetDudeFireAngle(double* d_Angle);
RSPIX_DECLSPEC extern void GetDudeVelocity(double* d_Velocity, double* d_Angle);

// Functions to convert bitfields to joybutton numbers and back again.
RSPIX_DECLSPEC extern int16_t JoyBitfieldToIndex(U32 bitfield);
RSPIX_DECLSPEC extern U32 JoyIndexToBitfield(int16_t index);
RSPIX_DECLSPEC extern int16_t MouseBitfieldToIndex(U32 bitfield);
RSPIX_DECLSPEC extern U32 MouseIndexToBitfield(int16_t index);
...
RSPIX_DECLSPEC int32_t rspMacVideoProfile(			// Returns average time per update.
...
RSPIX_DECLSPEC void rspSetMacVideoUpdateOptions(
	int16_t	sMethod,								// In:  Method to be used
	int16_t sByteAlignment);					// In:  Byte alignment to be used

RSPIX_DECLSPEC void rspQueryVideoModeReset(void);

RSPIX_DECLSPEC int16_t rspQueryVideoMode(			// Returns 0 for each valid mode, then non-zero thereafter
...
RSPIX_DECLSPEC int16_t rspGetVideoMode(				// Returns 0 if sucessfull, non-zero otherwise
	int16_t* psDeviceDepth,					// Out: Device depth (unless NULL)
	int16_t* psDeviceWidth = NULL,			// Out: Device width (unless NULL)
	int16_t* psDeviceHeight = NULL,			// Out: Device height (unless NULL)
	int16_t* psDevicePages = NULL,			// Out: Maximum number of pages supported (unless NULL)
...
RSPIX_DECLSPEC int16_t rspSetVideoMode(				// Returns 0 if successfull, non-zero otherwise
...
RSPIX_DECLSPEC void rspKillVideoMode(void);

RSPIX_DECLSPEC int16_t rspSuggestVideoMode(		// Returns 0 if successfull, non-zero otherwise
...
RSPIX_DECLSPEC int16_t rspLockVideoPage(			// Returns 0 if successfull, non-zero otherwise
	void**	ppvMemory,						// Out: Pointer to video page or NULL
	int32_t*		plPitch);						// Out: Pitch of video page

RSPIX_DECLSPEC void rspUnlockVideoPage(void);

RSPIX_DECLSPEC int16_t rspLockVideoFlipPage(		// Returns 0 if successfull, non-zero otherwise
	void**	ppvMemory,						// Out: Pointer to video flip page or NULL
	int32_t*		plPitch);						// Out: Pitch of video flip page

RSPIX_DECLSPEC void rspUnlockVideoFlipPage(void);

RSPIX_DECLSPEC int16_t rspLockVideoBuffer(			// Returns 0 if successfull, non-zero otherwise
	void**	ppvBuffer,						// Out: Pointer to video buffer or NULL
	int32_t*		plPitch);						// Out: Pitch of video buffer

RSPIX_DECLSPEC void rspUnlockVideoBuffer(void);

RSPIX_DECLSPEC int16_t rspAllowPageFlip(void);	// Returns 0 if successfull, non-zero otherwise


RSPIX_DECLSPEC void rspCacheDirtyRect(
...
RSPIX_DECLSPEC void rspKeyRepeat(int bEnable);

RSPIX_DECLSPEC void rspPresentFrame(void);

RSPIX_DECLSPEC void rspUpdateDisplayRects(void);

RSPIX_DECLSPEC void rspUpdateDisplay(void);

RSPIX_DECLSPEC void rspUpdateDisplay(
...
RSPIX_DECLSPEC void rspSetPaletteEntry(
...
RSPIX_DECLSPEC void rspSetPaletteEntries(
...
RSPIX_DECLSPEC void rspGetPaletteEntries(
...
inline RSPIX_DECLSPEC void rspGetPaletteEntry(
	int16_t sEntry,								// In:  Palette entry (0 to 255)
...
RSPIX_DECLSPEC void rspLockPaletteEntries(
...
RSPIX_DECLSPEC void rspUnlockPaletteEntries(
	int16_t sStartEntry,						// In:  Starting entry (0 to 255)
	int16_t sCount);								// In:  Number of entries to do (1 to 256)
...
RSPIX_DECLSPEC void rspSetPaletteMaps(
	int16_t sStartEntry,						// In:  Starting destination entry (0 to 255)
	int16_t sCount,								// In:  Number of entries to do (1 to 256)
	uint8_t* pucRed,					// In:  Pointer to starting source red value
	uint8_t* pucGreen,				// In:  Pointer to starting source green value
...
RSPIX_DECLSPEC void rspGetPaletteMaps(
	int16_t sStartEntry,						// In:  Starting source entry (0 to 255)
	int16_t sCount,								// In:  Number of entries to do (1 to 256)
	uint8_t* pucRed,					// Out: Pointer to starting destination red value
	uint8_t* pucGreen,				// Out: Pointer to starting destination green value
...
RSPIX_DECLSPEC void rspUpdatePalette(void);

RSPIX_DECLSPEC void rspShieldMouseCursor(void);

RSPIX_DECLSPEC void rspUnshieldMouseCursor(void);
...
RSPIX_DECLSPEC int32_t rspGetMicroseconds(			// Returns time in microseconds
	int16_t sReset = FALSE);					// In:  TRUE to reset count, FALSE otherwise

RSPIX_DECLSPEC int32_t rspGetMilliseconds(void);	// Returns time in milliseconds

RSPIX_DECLSPEC S64 rspGetAppMicroseconds(void);	// Returns microseconds since app started
...
RSPIX_DECLSPEC int16_t rspSetSoundOutMode(				// Returns 0 if successfull, non-zero otherwise
...
RSPIX_DECLSPEC int16_t rspGetSoundOutMode(				// Returns 0 if successfull, non-zero otherwise
...
RSPIX_DECLSPEC void rspSetSoundOutBufferTime(
	int32_t lCurBufferTime);						// In:  New buffer time

RSPIX_DECLSPEC void rspKillSoundOutMode(void);		// Returns 0 if successfull, non-zero otherwise

RSPIX_DECLSPEC int16_t rspClearSoundOut(void);		// Returns 0 on success, non-zero otherwise

RSPIX_DECLSPEC int16_t rspPauseSoundOut(void);		// Returns 0 on success, non-zero otherwise

RSPIX_DECLSPEC int16_t rspResumeSoundOut(void);		// Returns 0 on success, non-zero otherwise

RSPIX_DECLSPEC int16_t rspIsSoundOutPaused(void);	// Returns TRUE if paused, FALSE otherwise

RSPIX_DECLSPEC int32_t rspGetSoundOutPos(void);		// Returns sound output position in bytes

RSPIX_DECLSPEC int32_t rspGetSoundOutTime(void);		// Returns sound output position in time

RSPIX_DECLSPEC int32_t rspDoSound(void);

RSPIX_DECLSPEC void rspLockSound(void);

RSPIX_DECLSPEC void rspUnlockSound(void);

////////////////////////////////////////////////////////////////////////////////
// Application API (Really CYAN, but requires too much integration in BLUE)
////////////////////////////////////////////////////////////////////////////////

RSPIX_DECLSPEC void rspSetApplicationName(
...
RSPIX_DECLSPEC int16_t rspIsBackground(void);			// Returns TRUE if in background, FALSE otherwise

RSPIX_DECLSPEC void rspSetBackgroundCallback(
	void (BackgroundCallback)(void));		// In:  Function to be called

RSPIX_DECLSPEC void rspSetForegroundCallback(
	void (ForegroundCallback)(void));		// In:  Function to be called


RSPIX_DECLSPEC int rspCommandLine(const char *cmd);
RSPIX_DECLSPEC void rspPlatformInit(void);
