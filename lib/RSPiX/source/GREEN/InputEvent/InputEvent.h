...
// rspClearKeyEvents().
RSPIX_DECLSPEC void rspClearAllInputEvents(void);	// Returns nothing.

// Clears any pending key input events.  Calls rspClearKeyEvents().
RSPIX_DECLSPEC void rspClearKeyInputEvents(void);	// Returns nothing.

// Clears any pending mouse input events.  Calls rspClearMouseEvents().
RSPIX_DECLSPEC void rspClearMouseInputEvents(void);	// Returns nothing.

// Gets the next input event.
RSPIX_DECLSPEC int16_t rspGetNextInputEvent(	// Returns 1 if there is an event, 0 if none.
...
RSPIX_DECLSPEC void ClearXInputState();

// Gets the next XInput state.
RSPIX_DECLSPEC int16_t GetXInputState(XInputState* xis); // Returns 1 on success, 0 if failure or controller not present.
RSPIX_DECLSPEC int16_t GetXInputStateNoUpdate(XInputState* xis);
RSPIX_DECLSPEC int16_t GetLastXInputState(XInputState* xis);	// Returns the last known XInputState

// Returns 1 if any button on the controller is pressed.
RSPIX_DECLSPEC int16_t IsXInputButtonPressed();
