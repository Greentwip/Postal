...
//
///////////////////////////////////////////////////////////////////////////////
void RSPIX_DECLSPEC rspSetWin32StaticColors(
...
RSPIX_DECLSPEC extern int16_t rspMsgBox(	// Returns one of the macro defined RSP_MB_RET_*.
...
//
////////////////////////////////////////////////////////////////////////////////
RSPIX_DECLSPEC int16_t rspOpenBox(								// Returns 0 if successfull, non-zero otherwise
...
//
////////////////////////////////////////////////////////////////////////////////
RSPIX_DECLSPEC int16_t rspSaveBox(								// Returns 0 if successfull, non-zero otherwise
	const char* pszBoxTitle,							// In:  Title of box
	const char*	pszDefaultPath,						// In:  Default directory and file
	char* pszSelectedFile,								// Out: File that user selected
	int16_t sSelectedFileBufSize,						// In:  Size of buffer pointed to by pszSelectedFile
...
RSPIX_DECLSPEC int16_t rspExec(			// Returns 0 on success.
...
RSPIX_DECLSPEC int16_t rspGetTempPath(			// Returns 0 on success, non-zero otherwise
	char* pszPath,							// Out: Temp path returned here if available.
	int16_t	sMaxPathLen);					// In:  Max path length (to avoid overwrites)

RSPIX_DECLSPEC char* rspPathToSystem(			// Returns pszSystem
	const char* pszRSPiX,				// In:  RSPiX path
	char* pszSystem);						// Out: System path

RSPIX_DECLSPEC char* rspPathToSystem(			// Returns pointer to system-specific path (static!!!)
	const char* pszRSPiX);				// In:  RSPiX path

RSPIX_DECLSPEC char* rspPathFromSystem(		// Returns pszSystem
	const char* pszRSPiX,				// In:  RSPiX path
	char* pszSystem);						// Out: System path

RSPIX_DECLSPEC char* rspPathFromSystem(		// Returns pointer to system-specific path (static!!!)
	const char* pszRSPiX);				// In:  RSPiX path
...
RSPIX_DECLSPEC int16_t rspSetFileAttributes(	// Returns 0 on success, non-zero otherwise
...
RSPIX_DECLSPEC int16_t  rspGetQuitStatus(void);	// Returns TRUE if quit detected, FALSE otherwise

RSPIX_DECLSPEC void  rspSetQuitStatus(
...
RSPIX_DECLSPEC void rspSetQuitStatusFlags(
...
RSPIX_DECLSPEC int16_t rspSetPrinterResolution(		// Returns 0 if successfull, non-zero otherwise
...
RSPIX_DECLSPEC int16_t rspPrinterPageSetupDialog(void);	// Returns 0 if successfull, non-zero otherwise

RSPIX_DECLSPEC int16_t rspPrinterPrintDialog(void);	// Returns 0 if successfull, non-zero otherwise

RSPIX_DECLSPEC int16_t rspGetPrinterResolution(		// Returns 0 if successfull, non-zero otherwise
	int32_t* plHorzRes,								// Out: Horizontal resolution (dpi)
	int32_t* plVertRes);								// Out: Vertical resolution (dpi)

RSPIX_DECLSPEC int16_t rspGetPrinterPageSize(			// Returns 0 if successfull, non-zero otherwise
	int32_t* plPageWidth,							// Out: Page width (in dots)
	int32_t* plPageHeight);							// Out: Page height (in dots)

RSPIX_DECLSPEC int16_t rspGetPrinterPageRotation(	// Returns 0 if successfull, non-zero otherwise
	int16_t* psRotation);							// Out: Rotation (0, 90, or 270 degrees)

RSPIX_DECLSPEC int16_t rspGetPrinterPageMargins(		// Returns 0 if successfull, non-zero otherwise
...
RSPIX_DECLSPEC int16_t rspGetPrinterJob(				// Returns 0 if successfull, non-zero otherwise
...
RSPIX_DECLSPEC int16_t rspStartPrinterDoc(void);		// Returns 0 if successfull, non-zero otherwise

RSPIX_DECLSPEC void rspEndPrinterDoc(void);

RSPIX_DECLSPEC int16_t rspStartPrinterPage(void);	// Returns 0 if successfull, non-zero otherwise

RSPIX_DECLSPEC int16_t rspEndPrinterPage(void);

RSPIX_DECLSPEC int16_t rspPrintToPage(					// Returns 0 if successfull, non-zero otherwise
