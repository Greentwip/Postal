...
#else
		menu->ami[i].pszText = _strdup(gamename);
#endif
	}
	...
	if (sResult == 0)
		...
		if (sResult == 0)
			...
			if (g_GameSettings.m_sTrickySystemQuit != FALSE)
				...
				rspSetQuitStatusFlags((signed int)(RSP_GKF_SHIFT));
				}
	...
	if (strlen(pszBasePath) > 0)
		...
		if ((strlen(pszBasePath) + 1) <= sMaxPathLen)
			...
			if (pszOrigDir != NULL)
				...
				Windows::Storage::StorageFolder^ acquired_folder = nullptr;

				std::thread([&data_folder, &acquired_folder, pszBasePath]() {
					try {

						std::string search_path_string = pszBasePath;

						std::wstring intermediate_path_string(search_path_string.begin(), search_path_string.end());

						auto search_path = ref new Platform::String(intermediate_path_string.c_str());

						auto file_task = create_task(data_folder->GetFolderAsync(search_path));
						file_task.wait();
						data_folder = file_task.get();
					}
					catch (Platform::COMException^ e) {
						acquired_folder = data_folder;
					}
				}).join();



				Platform::String^ installed_display_path = data_folder->Path;
				std::wstring installed_path_temp(installed_display_path->Begin());

				std::string installed_path(installed_path_temp.begin(), installed_path_temp.end());

				snprintf(pszBasePath, sMaxPathLen, "%s", installed_path.c_str());
